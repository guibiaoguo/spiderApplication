...
public class ProxyPool {
    ...
    private String proxyFile = "d:/data/lastUse.proxy";
    ...
    public static List<String[]> getProxyList() {
        ...
        try {
            br = new BufferedReader(new FileReader(new File("d:/data/proxy.txt")));
...
ProxyPool proxyPool = new ProxyPool();
        List<String[]> proxs = getProxyList();
        for (String[] a :proxs) {
            HttpHost httpHost = new HttpHost(a[0],Integer.parseInt(a[1]));
            proxyPool.returnProxy(httpHost,200);
        }
//        while (true) {
